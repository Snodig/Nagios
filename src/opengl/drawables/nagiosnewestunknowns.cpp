...
rpnoc::NagiosNewestUnknowns::NagiosNewestUnknowns( float windowSize, QWidget *parent )
	: DrawableSource( "Nagios - Newest Unknowns", "NAGIOS", windowSize, parent )
{
	//
}

rpnoc::NagiosNewestUnknowns::~NagiosNewestUnknowns( void )
{
	//
}

void rpnoc::NagiosNewestUnknowns::init( void )
{
...
void rpnoc::NagiosNewestUnknowns::draw()
{
	...
	for( int i = 0; i < mTable->getSize(); ++i ) {
		...
		mTextCellStatus[i]->setText(
				((NagiosSource*)mDataSource)->getAllOfStatus("UNKNOWN")[i][NagiosSource::SERVER_STATUS] );
		mTextCellMessage[i]->setText(
				((NagiosSource*)mDataSource)->getAllOfStatus("UNKNOWN")[i][NagiosSource::OUTPUT] );

		//Color me brimstone-orange
